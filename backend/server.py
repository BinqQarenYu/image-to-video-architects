...
def is_safe_url(url: str) -> bool:
    """Check if a URL is safe for internal requests (SSRF protection)."""
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                return False
        # Block sensitive ranges, allow loopback for local Ollama
        if ip.is_loopback:
            return True
...
async def upload_audio(file: UploadFile = File(...)):
    try:
        file_ext = Path(file.filename).suffix
        unique_filename = f"{uuid.uuid4()}{file_ext}"
        file_path = safe_join(AUDIO_DIR, unique_filename)
        with open(file_path, "wb") as buffer:
            shutil.copyfileobj(file.file, buffer)
    ...
    try:
        if request.provider == "openai":
            ...
            if not is_safe_url(endpoint):
                raise HTTPException(status_code=400, detail="Insecure Ollama endpoint provided")
...
async def compile_video(
    ...
    try:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            output_path = safe_join(VIDEOS_DIR, output_filename)
