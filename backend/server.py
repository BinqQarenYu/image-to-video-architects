...
async def _prepare_image(idx: int, url: str, width: int, height: int, temp_path: Path):
    """Worker to process a single image in a thread-safe manner."""
    def _inner():
        ...
        # Use BILINEAR for speed over LANCZOS (~40% gain)
...
async def generate_video(
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Parallelize image processing using asyncio.gather and threads
            tasks = [
                _prepare_image(idx, url, width, height, temp_path)
