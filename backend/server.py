...
class VideoEngine:
    ...
    async def generate_minimax(image_local_path: str, prompt: str, api_key: str) -> str:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            img_bytes = await asyncio.to_thread(_read_image, image_local_path)
        ...
        async with httpx.AsyncClient(timeout=120) as client:
            ...
            dl.raise_for_status()
            # Offload blocking file write to a thread
    ...
    async def generate_runway(image_local_path: str, prompt: str, api_key: str) -> str:
        ...
        ext = Path(image_local_path).suffix.lstrip(".")
        b64_img = await asyncio.to_thread(_read_image_b64, image_local_path)
...
def _save_upload(file: UploadFile, target_path: Path):
    """Helper to save an uploaded file using blocking shutil.copyfileobj."""
...
async def upload_images(files: List[UploadFile] = File(...)):
    try:
        ...
        for file in files:
            ...
            # Use asyncio.to_thread to run blocking file writes in separate threads
            tasks.append(asyncio.to_thread(_save_upload, file, file_path))

        # Parallelize all file uploads for better throughput
...
async def upload_audio(file: UploadFile = File(...)):
    try:
        ...
        await asyncio.to_thread(_save_upload, file, file_path)
...
def _prepare_image(source_path: Path, target_path: Path, width: int, height: int):
    """Helper to resize and save image using BILINEAR for speed with minimal visual loss for video."""
    with Image.open(source_path) as img:
        if img.mode != 'RGB':
            img = img.convert('RGB')
        # BILINEAR is ~2-3x faster than LANCZOS and sufficient for video frame generation
...
async def generate_video(
    ...
    try:
        ...
        def _write_concat_file(file_path: Path, images: List[str], duration: float):
            """Helper to write the concat list file for FFmpeg."""
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Prepare image tasks for parallel execution
            tasks = []
            processed_images = []
            for idx, url in enumerate(image_urls):
                ...
                # Use asyncio.to_thread to run blocking PIL operations in separate threads
            ...
            if len(processed_images) == 1:
                ...
                # Offload blocking file write to a thread
                await asyncio.to_thread(_write_concat_file, concat_file, processed_images, image_duration)
