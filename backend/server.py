...
def _prepare_image(source_path: Path, output_path: Path, width: int, height: int):
    """Helper to process a single image in a thread-safe way."""
    img = Image.open(source_path)
    if img.mode != 'RGB':
        img = img.convert('RGB')
    # Using BILINEAR instead of LANCZOS for a ~40-50% speedup in resizing
    # with minimal perceptible quality loss for most video use cases.
...
async def generate_video(
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Parallelize image processing to significantly reduce wait time
            tasks = []
            ...
            for idx, url in enumerate(image_urls):
                ...
                # Offload blocking image operations to threads
