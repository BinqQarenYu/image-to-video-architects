...
def _prepare_image(source_path: Path, processed_path: Path, width: int, height: int):
    """Worker function to process a single image for the slideshow."""
    with Image.open(source_path) as img:
        if img.mode != 'RGB':
            img = img.convert('RGB')
        # Optimization: Use BILINEAR resampling instead of LANCZOS.
        # It's significantly faster (~3-4x) with negligible quality loss for video frames.
...
async def generate_video(
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # ⚡ OPTIMIZATION: Process all images in parallel using asyncio.gather
            # This offloads the CPU-heavy image resizing to threads to avoid blocking the event loop.
            tasks = []
