...
class VideoEngine:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            def read_img():
                with open(image_local_path, "rb") as f:
                    return f.read()
            img_bytes = await asyncio.to_thread(read_img)
    ...
    async def generate_runway(image_local_path: str, prompt: str, api_key: str) -> str:
        ...
        b64_img = await asyncio.to_thread(read_b64)
...
async def generate_video(
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            tasks = [asyncio.to_thread(process_image, idx, url) for idx, url in enumerate(image_urls)]
